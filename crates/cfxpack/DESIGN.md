...
| `timestamp` | ULEB128 | 相对于 `min_timestamp` 的偏移 |
...
| `pos_view` | ULEB128 | 相对于 `min_pos_height` 的偏移，仅相关 cip 激活后有该字段 |
...
数据块的位置由区块编码中的 `tx_segment_offset × 4` 确定（相对于交易段起始位置）。`tx_compressed` 标志指示 `payload` 是否经过压缩；压缩仅在 RLP 编码后长度超过 512 字节时启用。
...
- `epoch_height`（target epoch）：不记录绝对值，记录相对于交易所在区块 epoch 的**带符号偏移**（`epoch_height − block_epoch`，可正可负）。该偏移经 ZigZag 映射为无符号整数后再 RLP 编码，故解码时必须反向 ZigZag 还原符号，不能简单相加。
...
| `pos_reference_enable_height` | 主网 37400000 | 低于此高度的区块 `pos_reference` 视为 None，`finalized_epoch` 记 0（见 §8.6） |
| `cip112_transition_height` | 主网 79050000 | 区块头 `custom` 字段编解码规则切换点；解码 ≥ 此高度的区块头必须设置正确，否则派生字段错误 |
...
| `tx_segment_offset` | 编码时计算（该区块交易数据块在交易段中的位置 ÷ 4） |
...
4. 编码值 = 当前区块所在 epoch 编号 − `pd_height`。

> **不减 defer。** conflux 执行时 `env.finalized_epoch` 直接取 pivot decision 区块的 height（见 consensus_executor `epoch_execution.rs`：`finalized_epoch: pivot_decision_epoch`，即 `header.height()`），**不再减 `pos_pivot_decision_defer_epoch_count`**——defer 已经在 PoS 层选择 pivot decision 时生效，这里再减就错了。因此本字段与 CIP-113 的 defer 切换（50→20）无关，提取器也不需要 `pos_pivot_decision_defer_epoch_count` / `cip113_transition_height`。解码侧 `pivot.epoch − 偏移` 即还原出 `pd_height`。
...
- `epoch_height`（仅 Core Space）：编码为相对于所在区块 epoch 的带符号偏移（`epoch_height − block_epoch`），经 ZigZag 映射后再 RLP 编码（见 §5.4 说明）
